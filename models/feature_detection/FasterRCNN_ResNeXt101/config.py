model = dict(
    ...
    rpn_head=dict(
        ...
        anchor_generator=dict(
            ...
            scales=[8],
            ratios=[0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0],
...
train_pipeline = [
    ...
    dict(type='Translate', level=1, max_translate_offset=153, prob=0.2),
...
data = dict(
    ...
    train=dict(
        type='CocoDataset',
        ann_file='data/coco/train/labels.json',
        img_prefix='data/coco/train/data',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', with_bbox=True),
    ...
    val=dict(
        type='CocoDataset',
        ann_file='data/coco/test/labels.json',
        img_prefix='data/coco/test/data',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(
                type='MultiScaleFlipAug',
...
seed = 11
total_epochs = 50
work_dir = 'models/feature_detection/FasterRCNN_ResNeXt101'
auto_resume = False
gpu_ids = [0]
