        ...
        **kwargs
        ...
        for lvl in range(num_levels):
            mlvl_pos_mask_targets[lvl] = torch.cat(
                mlvl_pos_mask_targets[lvl],
                dim=0,
            )
            mlvl_pos_mask_preds[lvl] = torch.cat(
                mlvl_pos_mask_preds[lvl],
                dim=0,
            )
        ...
        **kwargs
        ...
        for lvl in range(num_levels):
            ...
            mlvl_pos_mask_preds_x[lvl] = torch.cat(
                mlvl_pos_mask_preds_x[lvl],
                dim=0,
            )
            mlvl_pos_mask_preds_y[lvl] = torch.cat(
                mlvl_pos_mask_preds_y[lvl],
                dim=0,
            )
            mlvl_labels[lvl] = torch.cat(mlvl_labels[lvl], dim=0)
            temp_mlvl_cls_preds.append(
...
class DecoupledSOLOLightHead(DecoupledSOLOHead):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(DecoupledSOLOLightHead, self).__init__(*args, init_cfg=init_cfg, **kwargs)
