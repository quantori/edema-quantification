...
class GuidedAnchorHead(AnchorHead):
    ...
    def ga_loc_targets(self, gt_bboxes_list, featmap_sizes):
        ...
        for img_id in range(img_per_gpu):
            ...
            min_anchor_size = scale.new_full(
                (1,),
                float(anchor_scale * anchor_strides[0]),
            )
        ...
        loc_avg_factor = (
            sum(
                [t.size(0) * t.size(-1) * t.size(-2) for t in all_loc_targets],
            )
            / 200
        )
    ...
    def ga_shape_targets(
        ...
        bbox_anchors_list = images_to_levels(
            all_bbox_anchors,
            num_level_squares,
        )
