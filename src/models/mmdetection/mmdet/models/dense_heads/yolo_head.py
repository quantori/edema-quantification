...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(
        ...
        assert len(
            self.prior_generator.num_base_priors,
        ) == len(featmap_strides)
        self._init_layers()
    ...
    def _init_layers(self):
        ...
        for i in range(self.num_levels):
            ...
            conv_pred = nn.Conv2d(
                self.out_channels[i],
                self.num_base_priors * self.num_attrib,
                1,
            )
    ...
    def get_bboxes(
        ...
        for pred, stride in zip(pred_maps, self.featmap_strides):
            pred = pred.permute(0, 2, 3, 1).reshape(
                num_imgs,
                -1,
                self.num_attrib,
            )
        ...
        padding = flatten_bboxes.new_zeros(
            num_imgs,
    ...
    def loss_single(self, pred_map, target_map, neg_map):
        ...
        pred_map = pred_map.permute(
            0,
            2,
            3,
            1,
        ).reshape(num_imgs, -1, self.num_attrib)
