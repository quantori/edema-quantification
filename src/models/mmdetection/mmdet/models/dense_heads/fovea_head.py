...
class FoveaHead(AnchorFreeHead):
    ...
    def loss(
        ...
        if num_pos > 0:
            ...
            pos_weights = (
                pos_bbox_targets.new_zeros(
                    pos_bbox_targets.size(),
                )
                + 1.0
            )
    ...
    def _get_target_single(
        ...
        for base_len, (lower_bound, upper_bound), stride, featmap_size, points in zip(
            ...
            # scale assignment
            hit_indices = (
                ((gt_areas >= lower_bound) & (gt_areas <= upper_bound)).nonzero().flatten()
            )
            ...
            pos_left = (
                torch.ceil(
                    gt_bboxes[:, 0] + (1 - self.sigma) * half_w - 0.5,
                )
                .long()
            ...
            pos_top = (
                torch.ceil(
                    gt_bboxes[:, 1] + (1 - self.sigma) * half_h - 0.5,
                )
                .long()
