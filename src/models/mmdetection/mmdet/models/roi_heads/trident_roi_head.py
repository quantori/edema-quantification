...
from mmdet.core import bbox2result, bbox2roi, bbox_mapping, merge_aug_bboxes, multiclass_nms
    ...
    def simple_test(
        ...
        for i in range(len(img_metas) // num_branch):
            det_result = self.merge_trident_bboxes(
                torch.cat(
                    det_bboxes_list[i * num_branch : (i + 1) * num_branch],
                ),
                torch.cat(
                    det_labels_list[i * num_branch : (i + 1) * num_branch],
                ),
            )
