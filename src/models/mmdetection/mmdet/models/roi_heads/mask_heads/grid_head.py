...
class GridHead(BaseModule):
    ...
    def get_targets(self, sampling_results, rcnn_train_cfg):
        ...
        targets = torch.zeros(
            (num_rois, self.grid_points, map_size, map_size),
            dtype=torch.float,
        )
        ...
        for i in range(num_rois):
            ...
            for j in range(self.grid_points):
                ...
                cx = int(
                    (gridpoint_x - pos_bboxes[i, 0]) / pos_bbox_ws[i] * map_size,
                )
                cy = int(
                    (gridpoint_y - pos_bboxes[i, 1]) / pos_bbox_hs[i] * map_size,
                )
    ...
    def get_bboxes(self, det_bboxes, grid_pred, img_metas):
        ...
        bbox_res = torch.cat(
            [bboxes_x1, bboxes_y1, bboxes_x2, bboxes_y2, cls_scores],
            dim=1,
        )
