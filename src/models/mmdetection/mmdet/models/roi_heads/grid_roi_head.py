...
class GridRoIHead(StandardRoIHead):
    ...
    def forward_dummy(self, x, proposals):
        ...
        grid_feats = self.grid_roi_extractor(
            x[: self.grid_roi_extractor.num_inputs],
            grid_rois,
        )
    ...
    def _bbox_forward_train(
        ...
        grid_targets = self.grid_head.get_targets(
            sampling_results,
            self.train_cfg,
        )
    ...
    def simple_test(
        ...
        if grid_rois.shape[0] != 0:
            grid_feats = self.grid_roi_extractor(
                x[: len(self.grid_roi_extractor.featmap_strides)],
                grid_rois,
            )
            ...
            bbox_results = [
                [np.zeros((0, 5), dtype=np.float32) for _ in range(self.bbox_head.num_classes)]
