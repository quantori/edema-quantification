...
class PointRendRoIHead(StandardRoIHead):
    ...
    def _mask_forward_train(
        ...
        in training."""
        mask_results = super()._mask_forward_train(
            x,
            sampling_results,
            bbox_feats,
            gt_masks,
    ...
    def _mask_point_forward_train(
        ...
        loss_mask_point = self.point_head.loss(
            mask_point_pred,
            mask_point_target,
            pos_labels,
        )
    ...
    def _mask_point_onnx_export(self, x, rois, label_pred, mask_pred):
        ...
        for subdivision_step in range(self.test_cfg.subdivision_steps):
            ...
            if is_trt_backend:
                ...
                inds_dim0 = (
                    torch.arange(point_shape[0])
                    .reshape(
                        point_shape[0],
                        1,
                        1,
                    )
                    .expand_as(point_indices)
                )
                inds_dim1 = (
                    torch.arange(point_shape[1])
                    .reshape(
                        1,
                        point_shape[1],
                        1,
    ...
    def mask_onnx_export(self, x, img_metas, det_bboxes, det_labels, **kwargs):
        ...
        mask_pred = self._mask_point_onnx_export(
            x,
            mask_rois,
            det_labels,
            mask_pred,
