...
class SparseRoIHead(CascadeRoIHead):
    ...
    def simple_test(
        ...
        if all([proposal.shape[0] == 0 for proposal in proposal_list]):
            ...
            bbox_results = [
                [np.zeros((0, 5), dtype=np.float32) for i in range(self.bbox_head[-1].num_classes)],
            ] * num_imgs
            return bbox_results
        ...
        bbox_results = [
            bbox2result(det_bboxes[i], det_labels[i], num_classes) for i in range(num_imgs)
        ]
        ...
        if self.with_mask:
            ...
            for img_id in range(num_imgs):
                mask_pred_per_img = mask_pred[img_id].flatten(
                    0,
                    1,
                )[topk_indices]
