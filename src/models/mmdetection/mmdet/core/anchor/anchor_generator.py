...
class AnchorGenerator:
    ...
    def __init__(
        ...
        self.base_sizes = (
            [min(stride) for stride in self.strides] if base_sizes is None else base_sizes
        )
    ...
    def sparse_priors(
        ...
        priors = torch.stack([x, y, x, y], 1).to(dtype).to(device) + self.base_anchors[level_idx][
            base_anchor_id, :
        ].to(device)
