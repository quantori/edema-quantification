...
class CenterRegionAssigner(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None):
        ...
        if num_gts == 0 or num_bboxes == 0:
            ...
            assigned_gt_ids = is_bbox_in_gt_core.new_zeros(
                (num_bboxes,),
                dtype=torch.long,
            )
        ...
        if gt_bboxes_ignore is not None and gt_bboxes_ignore.numel() > 0:
            ...
            is_bbox_in_ignored_gts = is_located_in(
                bbox_centers,
                gt_bboxes_ignore,
            )
    ...
    def assign_one_hot_gt_indices(
        ...
        assigned_gt_inds = is_bbox_in_gt_core.new_zeros(
            (num_bboxes,),
            dtype=torch.long,
        )
        ...
        #  is saved
        pair_priority = is_bbox_in_gt_core.new_full(
            (num_bboxes, num_gts),
            -1,
            dtype=torch.long,
        )
