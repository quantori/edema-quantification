...
def bbox2bucket(
    ...
    for k in range(offset_topk):
        if k >= 1:
            offset_l_weights[
                inds,
                l_label[:, k],
            ] = (l_topk[:, k] < offset_upperbound).float()
            offset_r_weights[
                inds,
                r_label[:, k],
            ] = (r_topk[:, k] < offset_upperbound).float()
            offset_t_weights[
                inds,
                t_label[:, k],
            ] = (t_topk[:, k] < offset_upperbound).float()
            offset_d_weights[
                inds,
                d_label[:, k],
            ] = (d_topk[:, k] < offset_upperbound).float()
        else:
    ...
    side_num = int(np.ceil(num_buckets / 2.0))
    labels = torch.stack(
        [l_label[:, 0], r_label[:, 0], t_label[:, 0], d_label[:, 0]],
        dim=-1,
    )

    batch_size = labels.size(0)
    bucket_labels = (
        F.one_hot(labels.view(-1), side_num)
        .view(
            batch_size,
            -1,
        )
