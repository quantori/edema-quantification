...
class CocoPanopticDataset(CocoDataset):
    ...
    def evaluate_pan_json(
        ...
        for gt_ann in gt_json:
            ...
            if img_id not in pred_json.keys():
                raise Exception(
                    'no prediction for the image' ' with id: {}'.format(img_id),
                )
        ...
        if classwise:
            classwise_results = {
                k: v for k, v in zip(self.CLASSES, pq_results['classwise'].values())
            }
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        outfile_prefix = (
            os.path.join(tmp_dir.name, 'results') if tmp_dir is not None else jsonfile_prefix
        )
        if 'PQ' in metrics:
        ...
        if ('bbox' in metrics) or ('segm' in metrics) or ('proposal' in metrics):
            assert 'ins_results' in results[0], (
                'instance segmentation' 'results are absent from results'
            )

            assert self.ins_ann_file is not None, (
                'Annotation '
            ...
            eval_ins_results = self.evaluate_det_segm(
                results, result_files, coco_gt, metrics, logger, classwise, **kwargs
            )
...
def print_panoptic_table(pq_results, classwise_results=None, logger=None):
    ...
    if classwise_results is not None:
        class_metrics = [
            (name,) + tuple(f'{(metrics[k] * 100):0.3f}' for k in ['pq', 'sq', 'rq'])
