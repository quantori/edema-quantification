...
def main():
    ...
    with open(args.txt_path, 'r') as f:
        ...
        for i, cfg in enumerate(model_cfgs):
            ...
            # default setting
            if cfg.find('16x') >= 0:
                command_info = f'GPUS=16  GPUS_PER_NODE=8  ' f'CPUS_PER_TASK=2 {train_script_name} '
            elif cfg.find('gn-head_4x4_1x_coco.py') >= 0 or cfg.find('gn-head_4x4_2x_coco.py') >= 0:
                command_info = f'GPUS=4  GPUS_PER_NODE=4  ' f'CPUS_PER_TASK=2 {train_script_name} '
            else:
                command_info = f'GPUS=8  GPUS_PER_NODE=8  ' f'CPUS_PER_TASK=2 {train_script_name} '
            ...
            if max_keep_ckpts:
                command_info += (
                    f'--cfg-options ' f'checkpoint_config.max_keep_ckpts=' f'{max_keep_ckpts}' + ' '
                )
