...
def _construct_ann_info(h=427, w=640, c=3):
    bboxes = np.array(
        [
            [222.62, 217.82, 241.81, 238.93],
            [50.5, 329.7, 130.23, 384.96],
...
def _load_masks(results, poly2mask=True):
    ...
    if poly2mask:
        ...
        gt_masks = PolygonMasks(
            [_process_polygons(polygons) for polygons in gt_masks],
            h,
            w,
        )
...
def test_translate():
    ...
    def _check_bbox_mask(
        ...
        for key in results_translated.get('bbox_fields', []):
            ...
            if mask_key not in results:
                # e.g. 'gt_masks_ignore'
                continue
            masks, masks_translated = (
                results[mask_key].to_ndarray(),
            ...
            if direction == 'horizontal':
                ...
                if offset <= 0:
                    ...
                    gt_masks = np.concatenate(
                        (masks[:, :, -offset:], masks_pad),
                        axis=-1,
                    )
                else:
                    # right shift
                    gt_masks = np.concatenate(
                        (masks_pad, masks[:, :, :-offset]),
                        axis=-1,
                    )
            else:
                masks_pad = _pad(
    ...
    policies = [
        [
            dict(type='Translate', level=10, prob=1.0),
            dict(
                type='Translate',
                level=8,
                img_fill_val=img_fill_val,
