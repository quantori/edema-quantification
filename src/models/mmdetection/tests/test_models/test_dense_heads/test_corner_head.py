...
def test_corner_head_loss():
    ...
    assert empty_push_loss.item() == 0, 'there should be no push loss when there are no true boxes'
    assert empty_pull_loss.item() == 0, 'there should be no pull loss when there are no true boxes'
    assert empty_off_loss.item() == 0, 'there should be no box loss when there are no true boxes'
    ...
    for tl_emb_feat, br_emb_feat in zip(tl_embs, br_embs):
        tl_emb_feat[:, :, gt_bboxes_ind[0][1], gt_bboxes_ind[0][0]] = tl_emb_feat[
            :, :, gt_bboxes_ind[1][1], gt_bboxes_ind[1][0]
        ]
        br_emb_feat[:, :, gt_bboxes_ind[0][3], gt_bboxes_ind[0][2]] = br_emb_feat[
            :, :, gt_bboxes_ind[1][3], gt_bboxes_ind[1][2]
        ]
