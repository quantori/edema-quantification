...
def test_center_head_loss():
    ...
    assert loss_wh.item() == 0, 'there should be no loss_wh when there are no true boxes'
    assert loss_offset.item() == 0, 'there should be no loss_offset when there are no true boxes'
