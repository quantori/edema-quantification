...
def test_pisa_retinanet_head_loss():
    ...
    cfg = mmcv.Config(
        dict(
            ...
            isr=dict(k=2.0, bias=0.0),
            carl=dict(k=1.0, bias=0.2),
            allowed_border=0,
            pos_weight=-1,
            debug=False,
        ),
...
def test_pisa_roi_head_loss():
    ...
    # Anchor head expects a multiple levels of features per image
    feat = [torch.rand(1, 1, s // (2 ** (i + 2)), s // (2 ** (i + 2))) for i in range(1)]
    ...
    empty_gt_losses = self.forward_train(
        feat,
        img_metas,
        proposal_list,
        gt_bboxes,
    ...
    one_gt_losses = self.forward_train(
        feat,
        img_metas,
        proposal_list,
        gt_bboxes,
