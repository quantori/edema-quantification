...
def yolo_config():
    ...
    model = YOLOV3Head(
        num_classes=4, in_channels=[1, 1, 1], out_channels=[16, 8, 4], test_cfg=test_cfg, **head_cfg
    )
    model.requires_grad_(False)
    # yolov3 need eval()
    model.cpu().eval()
...
def test_fcos_head_onnx_export():
    ...
    centerness = [
        torch.rand(1, 1, s // feat_size, s // feat_size) for feat_size in [4, 8, 16, 32, 64]
    ]

    fcos_model.onnx_export = partial(
        fcos_model.onnx_export,
        img_metas=img_metas,
        with_nms=False,
    )
...
def test_ssd_head_forward():
    ...
    feats = [
        torch.rand(
            1,
            ssd_model.in_channels[i],
            featmap_size[i],
            featmap_size[i],
        )
        for i in range(len(ssd_model.in_channels))
    ]
