...
def test_avoidoom():
    ...
    if torch.cuda.is_available():
        ...
        result = AvoidCudaOOM.retry_if_cuda_oom(torch.mm)(
            tensor,
            tensor.transpose(
                1,
                0,
        ...
        assert (
            default_result.device == result.device
            and default_result.dtype == result.dtype
            and torch.equal(default_result, result)
        )
        ...
        assert (
            default_result.device == result.device
            and default_result.dtype == result.dtype
            and torch.allclose(default_result, result, 1e-3)
        )
        ...
        assert (
            result.dtype == default_result.dtype
            and result.device == default_result.device
            and torch.allclose(default_result, result)
        )
        ...
        assert result.dtype == default_result.dtype and result.device == default_result.device
