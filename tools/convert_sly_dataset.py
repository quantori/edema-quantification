...
def check_dataset_dirs(dataset_dir: str, dataset_ann_dir: str, dataset_img_dir: str) -> None:
    """
...
def crop_images(dataset_img_dir: str, save_dir: str) -> None:
    ...
    for img_name in img_list:
        ...
        subject_id, study_id, left_width, right_width, ext = img_name.replace('.', '_').split('_')
...
def prepare_metadata_annotations(dataset_ann_dir: str, save_dir: str) -> None:
    ...
    for ann_name in ann_list:
        ...
        annotation = pd.DataFrame(columns=['edema id', 'figure id', 'x1', 'y1', 'x2', 'y2'])

        ann = sly.io.json.load_json_file(os.path.join(dataset_ann_dir, ann_name))

        edema_name = get_edema_name(ann)

        subject_id, study_id, width, right_width, ext, _ = ann_name.replace('.', '_').split('_')
        height = ann['size']['height']
        cropped_img_path = os.path.join(save_dir, 'img', f'{subject_id}_{study_id}.{ext}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_dir', default='dataset/MIMIC-CXR-Edema-SLY/DS1', type=str)
